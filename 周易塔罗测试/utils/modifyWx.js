...
wx.rpx2px = function (rpx) {
  return (this.windowWidth || (this.windowWidth = this._getSystemInfoSync().windowWidth)) / 750 * rpx;
}
/**
* px单位转rpx
*@param px 需要转换的单位数值
...
wx.px2rpx = function (px) {
  return 750 / (this.windowWidth || (this.windowWidth = this._getSystemInfoSync().windowWidth)) * px;
}
...
* Promise化，失败后间隔time秒后重新尝试直到reCount次失败后抛出错误
**/
wx._getStorage = function (obj, time = 700, reCount = 3) {
  return this.promiseApi('getStorage', obj).catch(e => {
    var count = 0;
    function loop() {
      count++;
      return wx.delay(time).then(() => {
        return wx.promiseApi('getStorage', obj).catch(e => {
          if (count < reCount) {
            return loop()
          }
          // throw e;
...
/**
* 捕获错误，并尝试reCount次失败后返回false
**/
wx._setStorageSync = function (key, value, reCount = 3) {
  var count = 0;
  while (count < reCount) {
    try {
